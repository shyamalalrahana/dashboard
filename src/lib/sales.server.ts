...
export const createSale = createServerFn({ method: "POST" })
  ...
  .handler(async (ctx): Promise<SaleRecord> => {
    ...
    const [{ maxNum }] = await db
      ...
      .from(retailSales);
    const next = maxNum ? Number(String(maxNum).replace("SAL-", "")) + 1 : 1;
    ...
    if (data.items.length > 0) {
      ...
      // SQLite spells Postgres' greatest() as max().
      for (const it of data.items) {
        if (it.productId) {
          await db.update(products)
            .set({ stock: sql`max(${products.stock} - ${it.qty}, 0)` })
...
export const fetchSaleProducts = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      effectivePrice,
      offerEnabled: Boolean(r.offerEnabled) && offerValue > 0,
